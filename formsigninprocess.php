...
<?php  /////////////////////////////////////////////get and route session for institutions,users,admin
     if(isset($_SESSION["insti_user_id"])) { ?>
        <script type="text/javascript">
          window.location = "<?php echo '../view/aledah-ventures-kenya';?>";
        </script> <?php
          exit();
      }
   ...
   //  if(isset($_SESSION["CATEGORYpupil"])) { ?>
  	 <script type="text/javascript">
  	// window.location = "<?php echo '../view/dashboarduser'?>";
  	// </script><?php
   //    exit();
   //  }
   ...
   //      exit();
   //    } 
    if(isset($_SESSION["institution_type_PSU"])) { ?>
      <script type="text/javascript">
        window.location = "<?php echo '../view/institutions';?>";
      </script> <?php
        exit();
      } 
    if(isset($_SESSION["membership_no"])) { ?>
    ...
    // if(isset($_SESSION["admin_membershipno"])) { ?>
       <script type="text/javascript">
    //     window.location = "<?php echo '../view/admindashboard'; ?>";
    //   </script> <?php
    //     exit();
    //   } 
      ...
      $numervalidation = "/^[0-9]+$/"; // num validation
        if(empty($getthisemail) || empty($getpassword)) {
              echo " <div class='alert alert-danger'>
                <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><b>Please fill, company ID and password</b>
                </div> ";
                exit();
              } else {
                }
        //institution-user login proccessing done here 
              ...
              //$resultsetzero = preg_replace('#[^0-9]#']); 
                          if($existcount_resultset > 0) {
                            ...
                            if(password_verify($getpassword, $pass)) {
                                if($getthisemail == $membershipnousr && $pass == $usrpassword) {
                                   ...
                                   <script type="text/javascript">
                                       window.location = "../view/aledah-ventures-kenya?usrd=<?php echo $user_id;?>&instid=<?php echo $insti_id;?>&instimbsno=<?php echo $membershipnousr;?>&usr=<?php echo $firstname.'nbsp'.$lastname;?>";
                                   </script> 
                                <?php  
                                    } else {
                                  echo "<div class='alert alert-danger'>
